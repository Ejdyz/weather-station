...
const Icon = ({light,raining, lowestTemp}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      const result1 = await getHowCloudyCurrentlyIs({light: light});
      const result2 = await  getHowMuchIsCurrentlyRaining({rain: raining, temperature: lowestTemp})
      setLoading(false)
