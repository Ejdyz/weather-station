...
export default async function RecentHistory({sunsetDate = defaultSunsetDate, sunriseDate = defaultSunriseDate}: {sunsetDate: Date, sunriseDate: Date}) {
  ...
  function HistoryItem({record}: {record: typeof historyData[number]}) {
    ...
    const weatherCondition = getIconSrcFromWeatherData(
      ...
      },
      {
        ...
        data: formatWeatherData("temperature", apparentTemperature(record?.temperature || 0, record?.humidity || 0, record?.wind_speed|| 0)),
      },
      {
      ...
      },
      {
        ...
        data: formatWeatherData("pressure", pressureAtSeaLevel(record?.pressure || 0, record?.temperature || 0)),
      },
      {
      ...
      },
      {
        ...
        data: formatWeatherData("wind_direction", record?.wind_direction),
      },
      {
        name: t("data.rain"),
  ...
  return (
    <>
      <h3 className='text-lg font-bold text-white capitalize'>{formatDateToDisplayWrittenDays(new Date())}</h3>
  <div className='w-full flex flex-row-reverse gap-2 overflow-auto hover-scrollbar animate-all duration-100 '>
